  ...
  return (
    <div className="loginPage">
      <div className="login">
        ...
        {validationError && (
          <>
            <Alert severity="error">Invalid Username/Password.</Alert> <br />
          </>
        )}
        <form
          onSubmit={(evt) => {
            ...
            handleLogIn(evt, formData).then((res) => {
              ...
              if (res.error) {
                ...
                //if no error, set userData and isLoggedIn in local storage
                setUserData(res);
                setIsLoggedIn((login) => {
                  localStorage.setItem("isLoggedIn", true);
                  return true;
                });
                //reset error banner on sign in if there was a 401 error
                if (error.logout) {
                  setError(false);
                }
        ...
        >
          ...
          <Button variant="contained" fullWidth type="Submit">
            <VpnKey fontSize="small" sx={{ marginRight: "5px" }} />
            SIGN IN
          </Button>
          <p>
