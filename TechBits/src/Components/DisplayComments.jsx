...
export default function DisplayComments({ comments, setError, setComments }) {
  const user = {
    name: comments.author.name.first + " " + comments.author.name.last,
  };
  ...
  async function handleDelete(evt) {
    ...
    if (id) {
      commentDelete(id).then((res) => {
        if (!res.error) {
          return setComments((currentData) => {
            return currentData.filter((el) => {
              if (el._id !== id) {
                return el;
              }
            });
  ...
  return (
    <div className="displayCommentsWrapper">
      <Avatar sx={{ width: 44, height: 44, fontSize: "large" }}>
        {getInitials(user)}
      </Avatar>
      <div className="comment">
        <p className="commentDetails">
          {isAuthor ? "You" : comments.author.username}{" "}
          <span className="commentDate">{convertDate(comments.createdAt)}</span>
        </p>
