...
export default function CommentBox({ postId, setComments, setError }) {
  ...
  return (
    <div className="CommentBox">
      <form
        onSubmit={(evt) => {
          commentSubmit(evt, postId, commentBox).then((res) => {
            if (!res.error) {
              setComments((currentData) => {
                return [...currentData, res.comment];
              });
      ...
      >
        <InputLabel htmlFor="comment">Add a new comment</InputLabel>
        <Input
          id="comment"
          startAdornment={
            <InputAdornment position="start">
              <Avatar sx={{ width: 24, height: 24, fontSize: "small" }}>
                {getInitials(getUserData())}
              </Avatar>
            </InputAdornment>
          }
